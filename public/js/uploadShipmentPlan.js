(async () => {
  ...
  document.getElementById('upload-form').addEventListener('submit', async (e) => {
    ...
    try {
      const { rows, warnings } = await parseShipmentPlanFile(file);
      if (rows.length === 0) {
        throw new Error('No valid rows found — make sure the file has a "Model" column with values.');
      }
      ...
      resultBox.innerHTML = `
        ...
        <ul>
          <li>Total rows read: <strong>${summary.totalRows}</strong></li>
