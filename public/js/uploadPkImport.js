// Upload PK Import — reuses the Combined Report file format/parser, but every
// customer created from this page is force-tagged RTM Category "PK Import"
// server-side (see process_pk_import_chunk), regardless of what the file's
// own RTM Category column says.
(async () => {
  const session = await requireAuth();
  if (!session) return;
  const profile = await getCurrentProfile();
  renderLayout('upload-pk-import', profile);
  ...
  document.getElementById('upload-form').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i += CHUNK) {
        ...
        const { error: chunkErr } = await supabaseClient.rpc('process_pk_import_chunk', {
          p_batch_id: batchId,
          p_rows: chunkRows
        });
      ...
      resultBox.innerHTML = `
        ...
        <div class="alert-warning">Every new customer from this file was tagged RTM Category "PK Import".</div>
