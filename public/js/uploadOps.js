(async () => {
  ...
  renderLayout('upload-ops', profile);
  ...
  document.getElementById('upload-form').addEventListener('submit', async (e) => {
    ...
    try {
      const { rows, warnings } = await parseOpsExportFile(file);
      if (rows.length === 0) {
        throw new Error('No valid iPhone IMEI rows found in this file.');
      }
