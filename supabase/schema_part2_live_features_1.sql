...
begin
  ...
  -- Full list, matching migration_psi.sql -- this file can run before OR
  -- after migration_psi.sql, and re-running it must never narrow the
  -- constraint back down and break existing PSI upload_type rows.
  alter table upload_batches
    add constraint upload_batches_upload_type_check
    check (upload_type in (
      'ops_export', 'activation_check', 'combined_report', 'pk_import',
      'sales_ledger', 'purchase_ledger', 'inventory_snapshot', 'shipment_plan'
    ));
end $$;
...
-- ============================================================
-- get_imei_sales_trend(p_group_by, p_periods)
-- Units-only sales trend sourced from imei_records (the main IMEI
-- Sales / Combined Report data), for the main Dashboard's Sales Trend
-- panel. Mirrors get_sales_trend() in migration_psi.sql but reads
-- imei_records instead of sales_transactions -- imei_records has no
-- revenue/price column, so this is qty-only, no Units/Revenue toggle.
-- ============================================================
create or replace function get_imei_sales_trend(
...
begin
  ...
  with filtered as (
    select r.date_of_shipment as sale_date,
           coalesce(r.qty, 1) as qty,
           coalesce(nullif(trim(r.lob),''),'Unknown') as lob
    from imei_records r
    where r.date_of_shipment is not null
  ...
  select jsonb_build_object(
    ...
    'byLob', (
      select coalesce(jsonb_agg(jsonb_build_object('lob', lob, 'qty', qty)), '[]'::jsonb)
      from by_lob
    )
  ) into v_result;
